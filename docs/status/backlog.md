...
## Erledigt in v39 — Redis entfernt
**Umgesetzt:** Der ungenutzte Redis-Dienst ist aus dem Stack verschwunden,
samt `REDIS_URL` und `depends_on`. Cache und Sessions bleiben im
Dateisystem; welches Backend im Betrieb genutzt wird, entscheidet das
übernehmende Team — Begründung in ADR-13, Rezept in `handoff.md`. Neu ist
Abschnitt 20 in `verify.sh`: jeder Host aus einer DSN muss ein in
`docker-compose.yaml` deklarierter Dienst sein. Damit ist P3 erledigt.
